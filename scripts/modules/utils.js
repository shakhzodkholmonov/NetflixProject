    ...
    ["movie", "tv"].map((type) =>
      Promise.all(
        ["popular", "top_rated"].map((id) =>
          fetch(`${BASE_PATH}/${type}/${id}?api_key=${API_KEY}&with_genres=16`).then((response) => response.json())
        )
      )
    )
  );
...
const ringRating = (vote) => {
  ...
  rating.querySelector("svg circle:last-child").style.setProperty("--percent", strokeOffset);
...
const paintCard = (data, card) => {
  ...
  card.innerHTML += `<div><h3>${data.title || data.name}</h3><p>${data.overview}</p></div>`;
...
export const createModal = (data) => {
  ...
  card.style.backgroundImage = data.backdrop_path
    ? `linear-gradient(to right, #191919 50%, #0002), url(${getMovieImage(data.backdrop_path, "w1280")})`
    : noBgImage;
